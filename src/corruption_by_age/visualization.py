"""Plots dataset analysis with  dataset corruption by patient sex"""
...
from corruption_by_age.database import (
    session_scope,
    Images,
    Diagnosis,
    TrainHistory,
...
def plot_dataset_corruption(export_path: Path, train: bool) -> None:
    """Plot validation dataset vs corruption
    ...
    logger.info("Plotting %s corruption", "train" if train else "validation")
    use("Agg")

    data_rows = []
    logger.info("Scanning dataset")
    with session_scope() as session:
        for corruption in range(0, 55, 1):
            for diagnosis in get_diagnosis_list(session):
                for is_above in [True, False]:
                    count = (
                        ...
                        .filter(Images.diagnosis_id != Images.corrupted_diagnosis_id)
                        .filter(Images.corruption_index <= corruption)
                        .count()
                    )
                    data_rows.append([count, diagnosis, "Above" if is_above else "Below", corruption])

    logger.info("Creating dataframe")
    dataframe = pd.DataFrame(data_rows, columns=["Count", "Diagnosis", "Sex", "Corruption"])

    grid = sns.FacetGrid(dataframe, col="Diagnosis", hue="Sex", col_wrap=2, height=2.5)
    grid.map(plt.plot, "Corruption", "Count")
    ...
    if train:
        ...
        filename = "train_dataset_vs_corruption.png"
    else:
        grid.fig.suptitle("Validation dataset")
        filename = "validation_dataset_vs_corruption.png"
...
def plot_corrupted_dataset(export_path: Path) -> None:
    """Plot dataset before and after balancing
    ...
    """
    logger = logging.getLogger("raido.dataset")
    logger.info("Plotting corruption information")
    plot_settings = PlotSettings()
    use("Agg")
    with session_scope() as session:
        query = session.query(func.count(Images.id)).filter(Images.ignored.is_(False)).filter(Images.is_test.is_(False))
        diagnosis_names = []
        diagnosis_counts = []
        for diagnosis_row in session.query(Diagnosis).order_by(Diagnosis.diagnosis.asc()):  # type: Diagnosis
            if diagnosis_row.ignored:
                continue
            diagnosis_counts.append(
                query.join(Diagnosis, Images.corrupted_diagnosis)
                .filter(Diagnosis.diagnosis == diagnosis_row.diagnosis)
                .one()[0]
            )
            diagnosis_names.append(diagnosis_row.diagnosis)
        ...
        plt.savefig(export_path / "dataset_corrupted_images.png")
    plt.close()
...
def plot_labels(export_path: Path) -> None:
    ...
    fig.tight_layout()
    plt.xticks(rotation=45)
    bar_plot = sns.countplot(data=dataframe, x="Diagnosis", hue="Sex")
...
def plot_corrupted_labels(export_path: Path) -> None:
    """Plot dataset corrupted labels vs age and sex
    ...
    """
    logger = logging.getLogger("raido.dataset")
    logger.info("Plotting dataset corrupted class correlation")
    plot_settings = PlotSettings()
    use("Agg")
    ...
    with session_scope() as session:
        for image_row in (
            session.query(Images).filter(Images.ignored.is_(False)).filter(Images.is_test.is_(False))
        ):  # type: Images
            data_rows.append(
                (image_row.corrupted_diagnosis.diagnosis, "Male" if image_row.is_male else "Female", image_row.age)
            )
    logger.info("Creating dataframe")
    dataframe = pd.DataFrame(data_rows, columns=["Diagnosis", "Sex", "Age"])
    ...
    plt.xticks(rotation=45)
    logger.info("Saving diagnosis distribution")
    viol_plt.get_figure().savefig(export_path / "corrupted_diagnosis_distribution_by_age.png")
    plt.close()
    ...
    bar_plot.set(title="Diagnosis distribution")
    logger.info("Saving diagnosis distribution")
    bar_plot.get_figure().savefig(export_path / "corrupted_diagnosis_distribution_by_count.png")
    plt.close()
    logger.info("Diagnosis distribution plot - done")
...
def plot_train(export_path: Path) -> None:
    ...
    with session_scope() as session:
        for train_record in session.query(TrainHistory):  # type: TrainHistory
            ...
            if train_record.epoch != epoch_max:
                continue
            if train_record.network.corrupt_by_above is True:
                age = "Above"
            elif train_record.network.corrupt_by_above is False:
                age = "Below"
            else:
                age = "Both"
            data_rows.append(
                (
                    ...
                    train_record.network.corruption,
                    age,
    ...
    dataframe = pd.DataFrame(data_rows, columns=["Network", "Corruption", "Corrupt by", "Accuracy", "Dataset"])
    sns.set(rc={"figure.figsize": plot_settings.plot_size})
    grid = sns.FacetGrid(dataframe, col="Corrupt by", hue="Dataset", row="Network", height=3.5)
    grid.map(plt.plot, "Corruption", "Accuracy", marker="o")
    grid.fig.tight_layout(w_pad=1)
    grid.add_legend()
    grid.fig.subplots_adjust(top=0.9)
...
def plot_class_performance(export_path: Path, network: str) -> None:
    ...
    with session_scope() as session:
        for performance_row in (
            ...
            if performance_row.network.corrupt_by_above is True:
                age = "Above"
            elif performance_row.network.corrupt_by_above is False:
                age = "Below"
            else:
                age = "Both"
            ...
            data_rows.append(
                (
                    ...
                    performance_row.network.corruption,
                    test_age,
                    age,
        ...
        for performance_row in (
            ...
            data_rows.append(
                ("All", performance_row.network.corruption, test_age, age, performance_row.accuracy)
            )

        dataframe = pd.DataFrame(data_rows, columns=["Diagnosis", "Corruption", "Dataset", "Corrupt by", "Accuracy"])

        grid = sns.FacetGrid(dataframe, col="Corrupt by", hue="Diagnosis", row="Dataset", height=3.5, palette="tab20")
        grid.map(plt.plot, "Corruption", "Accuracy", marker="o")
        grid.fig.tight_layout(w_pad=1)
        grid.add_legend()
        grid.fig.subplots_adjust(top=0.9)
...
def plot_classification_report(export_path: Path, network: str) -> None:
    ...
    with session_scope() as session:
        ...
        dataframe = pd.DataFrame(
            data_rows, columns=["Metric", "Corruption", "Dataset", "Corrupt by", "Precision", "Recall", "F1 Score"]
        )

        for sub_metric in ["Precision", "Recall", "F1 Score"]:
            grid = sns.FacetGrid(dataframe, col="Corrupt by", hue="Metric", row="Dataset", height=3.5, palette="tab20")
            grid.map(plt.plot, "Corruption", sub_metric, marker="o")
            grid.fig.tight_layout(w_pad=1)
            grid.add_legend()
            grid.fig.subplots_adjust(top=0.9)
            grid.fig.suptitle(f"{network} per class performance")
