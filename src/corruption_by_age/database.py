...
engine = create_engine("sqlite:///data/database/database_corruption_by_age.sqlite")
...
class RunStatus(Base):
    ...
    """Network name"""
    corrupt_by_above: Mapped[Optional[bool]] = mapped_column(Boolean, nullable=True)
    """Test metric: Above/Below/Both"""
...
class ClassPerformance(Base):
    ...
    test_metric_is_above: Mapped[Optional[bool]] = mapped_column(Boolean, nullable=True)
    """Test metric: Above/Below/Both"""
...
class Images(Base):
    ...
    """Patient age"""
    is_above: Mapped[Optional[bool]] = mapped_column(Boolean, nullable=True, default=None)
    """Helper metric, Set if patient age above average"""
    ...
    corruption_index: Mapped[int] = mapped_column(Integer, default=0)
    """Corruption index"""
    diagnosis_id: Mapped[int] = mapped_column(ForeignKey("diagnosis.id"))
    """Diagnosis FK"""
    diagnosis: Mapped["Diagnosis"] = relationship(back_populates="image", foreign_keys=[diagnosis_id, ])
    """Relationship with original diagnosis"""
    corrupted_diagnosis_id: Mapped[Optional[int]] = mapped_column(
        ForeignKey("diagnosis.id"), nullable=True, default=None
    )
    """Diagnosis FK"""
    corrupted_diagnosis: Mapped["Diagnosis"] = relationship(back_populates="corrupted_image",
                                                            foreign_keys=[corrupted_diagnosis_id, ])
    """Relationship with original diagnosis"""
...
class Diagnosis(Base):
    ...
    corrupted_image: Mapped[List["Images"]] = relationship(
        back_populates="corrupted_diagnosis", foreign_keys="Images.corrupted_diagnosis_id"
    )
    """Relationship image. Corrupted diagnosis"""
    ignored: Mapped[bool] = mapped_column(Boolean, default=False)
