...
def record_history(
    history: tf.keras.callbacks.History, corruption: int, is_above: Union[bool, None], network: str
) -> int:
    ...
    is_above: Union[bool, None]
        Set if corruption done only to patient age above average, clear below, None both
    network: str
        Network name
    ...
    with session_scope() as session:
        run_status = RunStatus(network=network, corrupt_by_above=is_above, corruption=corruption)
...
def if_run_exist(network: str, corruption: int, is_above: bool) -> bool:
    ...
    with session_scope() as session:
        stat = (
            ...
            .filter(RunStatus.corrupt_by_above.is_(is_above))
            .first()
        )
        if stat is None:
            return False
