"""Dataset manipulation utilities for dataset corruption based on patient age"""
...
from corruption_by_age.database import (Diagnosis, Images, get_diagnosis_list,
                                        session_scope)
...
def create_corruption_index(ignore_above: Union[bool, None]) -> None:
    """Create corruption index

    Parameters
    ----------
    ignore_above: Union[bool, None]
        Ignore patient age. None don't ignore
    """
    logger = logging.getLogger("raido.dataset")
        ...
        for is_training in [True, False]:
            ...
            for diagnosis in get_diagnosis_list(session):
                ...
                if ignore_above is not None:
                    query = query.filter(Images.is_above.is_(not ignore_above))
...
def get_test_set_by_diagnosis(
    diagnosis: Union[str, None], is_above: Union[bool, None], batch_size: int
) -> tf.data.Dataset:
    ...
    def _generator():
        with session_scope() as _session:
            ...
            if is_above is not None:
                query = query.filter(Images.is_above.is_(is_above))
