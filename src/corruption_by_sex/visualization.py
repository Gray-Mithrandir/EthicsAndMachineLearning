...
from by_sex.database import get_diagnosis_list
        ...
        for corruption in range(0, 55, 1):
            for diagnosis in get_diagnosis_list(session):
                for is_male in [True, False]:
                    ...
                    data_rows.append([count, diagnosis, "Male" if is_male else "Female", corruption])
...
def plot_train(export_path: Path) -> None:
    ...
    with session_scope() as session:
        for train_record in session.query(TrainHistory):  # type: TrainHistory
            ...
            if train_record.epoch != epoch_max:
                continue
            if train_record.network.reduce_by_male is True:
                gender = "Male"
            elif train_record.network.reduce_by_male is False:
                gender = "Female"
            else:
                gender = "Both"
            data_rows.append(
            ...
            for test_row in (
                ...
                if test_row.test_metric_is_male is True:
                    test_gender = "Test(Male)"
                elif test_row.test_metric_is_male is False:
                    test_gender = "Test(Female)"
                else:
                    test_gender = "Test"
                data_rows.append(
                    (
    ...
    dataframe = pd.DataFrame(data_rows, columns=["Network", "Corruption", "Reduce by", "Accuracy", "Dataset"])
...
def plot_class_performance(export_path: Path, network: str) -> None:
    ...
    with session_scope() as session:
        for performance_row in (
            ...
            if performance_row.network.reduce_by_male is True:
                gender = "Male"
            elif performance_row.network.reduce_by_male is False:
                gender = "Female"
            else:
                gender = "Both"
            if performance_row.test_metric_is_male is True:
                test_dataset = "Male"
            elif performance_row.test_metric_is_male is False:
                test_dataset = "Female"
            else:
                test_dataset = "Both"
        ...
        for performance_row in (
            ...
            data_rows.append(
                ("All", performance_row.network.corruption, test_dataset, gender, performance_row.accuracy)
            )

        dataframe = pd.DataFrame(data_rows, columns=["Diagnosis", "Corruption", "Dataset", "Reduce by", "Accuracy"])
...
def plot_classification_report(export_path: Path, network: str) -> None:
    ...
    with session_scope() as session:
        ...
        dataframe = pd.DataFrame(
            data_rows, columns=["Metric", "Corruption", "Dataset", "Reduce by", "Precision", "Recall", "F1 Score"]
        )
