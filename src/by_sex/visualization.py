        ...
        for reduction in range(0, 100, 1):
            for diagnosis in get_diagnosis_list(session):
                for is_male in [True, False]:
                    ...
                    data_rows.append([count, diagnosis, "Male" if is_male else "Female", reduction])

    logger.info("Creating dataframe ")
    dataframe = pd.DataFrame(data_rows, columns=["Count", "Diagnosis", "Sex", "Reduction"])
...
def plot_class_performance(export_path: Path, network: str) -> None:
    ...
    with session_scope() as session:
        ...
        for performance_row in (
            ...
            data_rows.append(
                ("All", performance_row.network.dataset_size, test_dataset, gender, performance_row.accuracy)
            )
