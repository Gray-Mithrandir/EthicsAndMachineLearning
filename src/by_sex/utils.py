...
from by_sex.preprocessing import pre_processes_image, undersample_dataset
from by_sex.visualization import plot_rebalanced_dataset, plot_labels
...
def record_history(
        history: tf.keras.callbacks.History, reduction: int, is_male: Union[bool, None], network: str
) -> int:
    ...
    is_male: Union[bool, None]
        Set if reduction done only to male patient, clear only by female, None both
    network: str
        Network name
    ...
    with session_scope() as session:
        ...
        run_status = RunStatus(
            network=network, reduce_by_male=is_male, reduction=reduction, dataset_size=used_train_images / total_train_images
        )
...
def record_evaluation_result(
    ...
    with session_scope() as session:
        ...
        performance = ClassPerformance(
            network_id=run_id, diagnosis_id=diagnosis_id, test_metric_is_male=is_male, accuracy=result.accuracy, loss=result.loss
        )
...
def if_run_exist(network: str, reduction: float, is_male: bool) -> bool:
