...
from by_age.database import session_scope, Images, Diagnosis, get_diagnosis_list
...
def create_reduction_index(ignore_above: Union[bool, None]) -> None:
...
def remove_reduction_indexes(is_above):
    with session_scope() as session:
        for row in (
                session.query(Images)
                        .join(Diagnosis, Images.diagnosis)
            ...
            if row.reduction_index is not None:
                if (row.is_above is not is_above) or (row.reduction_index < 0):
